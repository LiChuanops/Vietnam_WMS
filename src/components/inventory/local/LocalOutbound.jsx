...
const LocalOutbound = () => {
  ...
  // Define state for outbound data
  ...
  // 从父组件获取状态
  const { bulkProducts, productFilters, showProductList, transactionDate } = outboundData
  ...
  const updateTransactionDate = (date) => {
    updateOutboundData({ transactionDate: date })
  }
  ...
  const removeProductFromBulk = (index) => {
    ...
    updateOutboundData({ bulkProducts: renumbered })
  }

  const updateProductInBulk = (index, field, value) => {
    const product = bulkProducts[index];
    if (field === 'quantity' && product.current_stock && parseFloat(value) > product.current_stock) {
      alert(`${t('quantityCannotExceedStock')} (${product.current_stock})`);
      return;
    }
    const updated = bulkProducts.map((item, i) =>
      i === index ? { ...item, [field]: value } : item
    )
    updateOutboundData({ bulkProducts: updated })
  }
                ...
                <table className="min-w-full divide-y divide-gray-200">
                  <thead className="bg-gray-50 sticky top-0">
                    <tr>
                      ...
                      <th className="px-3 py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">{t('currentStock')}</th>
