...
const InventoryReports = () => {
  ...
  const exportShipmentToExcel = (shipment) => {
    // Simple CSV export for Excel compatibility
    const headers = ['S/N', 'Product Code', 'Product Description', 'Packing', 'Batch No', 'Quantity']
    const rows = shipment.transactions.map((transaction, index) => [
      index + 1,
    ...
    const csvContent = [
      ...
      `Container: ${shipment.container}, ETD: ${shipment.etd}, ETA: ${shipment.eta}`,
      '',
      headers.join(','),
    ...
    const url = URL.createObjectURL(blob)
    link.setAttribute('href', url)
    link.setAttribute('download', `shipment_${shipment.shipment}_${new Date().toISOString().split('T')[0]}.csv`)
    link.style.visibility = 'hidden'
    document.body.appendChild(link)
    link.click()
    document.body.removeChild(link)
  ...
  const printShipmentPDF = (shipment) => {
    ...
    const printContent = `
      ...
      <html>
        <head>
          <title>Shipment Details - ${shipment.shipment}</title>
          <style>
            body { font-family: Arial, sans-serif; margin: 20px; }
            .header { margin-bottom: 20px; }
            .info { display: grid; grid-template-columns: repeat(3, 1fr); gap: 10px; margin-bottom: 20px; }
            .info div { padding: 5px; background: #f5f5f5; }
        ...
        <body>
          <div class="header">
            ...
            <h2>${shipment.shipment}</h2>
          </div>
          
          <div class="info">
            <div><strong>Container:</strong> ${shipment.container}</div>
            <div><strong>Seal No:</strong> ${shipment.seal}</div>
            <div><strong>PO Number:</strong> ${shipment.po}</div>
            <div><strong>ETD:</strong> ${shipment.etd}</div>
            <div><strong>ETA:</strong> ${shipment.eta}</div>
            <div><strong>Total Products:</strong> ${shipment.transactions.length}</div>
          </div>
            ...
            <tbody>
              ${shipment.transactions.map((transaction, index) => `
                <tr>
                  ...
                  <td>${transaction.products?.packing_size || ''}</td>
              ...
              <tr class="total">
                ...
                <td><strong>${shipment.totalQuantity.toLocaleString()}</strong></td>
              </tr>
            </tbody>
          </table>
        </body>
